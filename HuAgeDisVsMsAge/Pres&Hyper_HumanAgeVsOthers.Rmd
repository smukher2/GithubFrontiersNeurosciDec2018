---
title: "Pres&Hyper_HumanAgeVsOthers"
...
dat1colors:'ForPres&Hyper_HuAgeDis_GeneModule.csv' and 'ForPres&Hyper_HuAgeDis_datExprA1g2.csv' from 'HumanAgeDis'

dat2labels, results output 'MsAge7K':'ForPres&Hyper_MsAge7K_GeneModuleL.csv' and 'ForPres&Hyper_MsAge7K_datExprA1g2.csv' from 'HumanAgeDis'
  ...
  b) replace "HuAgeDis" and "MsAge7K" with appropriate names for the dataset 
...
dat1colors=read.csv('./InputPreservation/ForPres&Hyper_HuAgeDis_GeneModule.csv', sep=',')
...
dat1Expr=read.csv('./InputPreservation/ForPres&Hyper_HuAgeDis_datExprA1g2.csv', sep=',')
...
#####################################################################for comparison with MsAge7K modules##################################################################################

###Step 3: Preparation of data for preservation analysis MsAge7K###
#Import dat2labels for analysis
```{r}
#Obtain this listMod of choice from the WGCNA pipeline to which the dataset needs to be compared with
dat2labels=read.csv('./InputPreservation/ForPres&Hyper_MsAge7K_GeneModuleL.csv', sep=',')
...
```{r}
# the following data frame contains the gene expression data: columns are genes, rows are arrays (samples)
dat2Expr=read.csv('./InputPreservation/ForPres&Hyper_MsAge7K_datExprA1g2.csv', sep=',')
...
###Step 4: Preservation analysis with dat1colors vs dat2labels MsAge7K###
...
```{r}
setLabels1 = c("HuAgeDis_colors", "MsAge7K_labels");
...
# Start the plot
sizeGrWindow(10, 5);
pdf(file="HuAgeDis_colors_vs_MsAge7K_labels_preservation.pdf", width=30, height=15)
par(mfrow = c(1,2))
...
write.csv(mp_Zsummary, "HuAgeDis_colors_vs_MsAge7K_labels_preservation_Zsummary.csv")
```
...
write.csv(mp_logp_Bonfsummary[,-c(1)], "HuAgeDis_colors_vs_MsAge7K_labels_preservation_logP_Bonfsummary.csv")
```

###Step 5: Enrichment test for comparison with dat1colors vs dat2labels MsAge7K ###
```{r}
...
write.csv(dat2labels_1,"ForEnrich_dat2MsAge7Klabels_Hu.csv")
```
...
#dat2labelsGeneCount
write.csv(dat2labelsGeneCount,"ForEnrich_dat2MsAge7Klabels_Hu_GeneCount.csv")
```

```{r}
#This is the actual hypergeometric enrichment test step
modulesA1=dat1colors
enrichmentsdat2labels = userListEnrichment(colnames(dat1Expr), modulesA1,"ForEnrich_dat2MsAge7Klabels_Hu.csv", "", "enrichmentsdat2MsAge7Klabels_colors.csv")
```

```{r}
#Saving results of enrichment
enrichmentsdat2labels_OvGenes=enrichmentsdat2labels[[2]]
dir.create("./enrichmentsdat2MsAge7Klabels_OvGenes")
for (i in 1:length(enrichmentsdat2labels_OvGenes)) {
  write.csv(enrichmentsdat2labels_OvGenes[i], file=paste0("enrichmentsdat2MsAge7Klabels_OvGenes/", names(enrichmentsdat2labels_OvGenes)[i], ".txt"))
}

enrichmentsdat2labels_NumOvGenes_pvalue=enrichmentsdat2labels[[1]]
write.csv(enrichmentsdat2labels_NumOvGenes_pvalue,"enrichmentsdat2MsAge7K_NumOvGenes_pvalue_colors.csv")
```
...
file.move(list.files(pattern = "enrichmentsdat2MsAge7Klabels_colors.csv"), "enrichmentsdat2MsAge7Klabels_OvGenes")
file.move(list.files(pattern = "enrichmentsdat2MsAge7K_NumOvGenes_pvalue_colors.csv"), "enrichmentsdat2MsAge7Klabels_OvGenes")
file.move(list.files(pattern = "ForEnrich*"), "enrichmentsdat2MsAge7Klabels_OvGenes")

dir.create("PreservationResults")
file.move(list.files(pattern = "HuAgeDis_colors_vs_MsAge7K_labels*"), "PreservationResults")
```
